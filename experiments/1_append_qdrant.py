...
async def main() -> bool:
    ...
    if len(full_embeddings_list) != len(texts_for_batch_embedding):
        logger.error(f"Error: Mismatch between number of texts ({len(texts_for_batch_embedding)}) "
    ...
    for i, full_embedding in enumerate(full_embeddings_list):
        ...
        if not isinstance(full_embedding, (list, np.ndarray)) or len(full_embedding) == 0:
            logger.warning(f"Warning: Invalid or empty embedding received for item with instruction: '{instruction_text[:50]}...'. Skipping.")
            continue
        ...
        for collection_name, target_dim in COLLECTION_CONFIGS.items():
            if len(full_embedding) < target_dim :
                 logger.error(f"Error: Base embedding (padded or not) is too short ({len(full_embedding)}d) for target dimension {target_dim} in {collection_name}. Skipping point.")
                 continue
        ...
        if processed_documents_count % QDRANT_BATCH_SIZE == 0 or i == len(full_embeddings_list) - 1:
            for col_name, points_list in points_to_upsert_batched.items():
                if points_list:
                    logger.info(f"Upserting batch of {len(points_list)} points to {col_name} (Total docs processed: {processed_documents_count})...")
                    try:
